                ...
                <div class="item">
                    <a href="albums/{{$album->id}}">
                      @if(empty($album->image))
                        <img src="{{asset('storage/uploads/5NTzOnBzMcxmUd8ZTNPgvmIV0Hi3Hj7C0tfdUzTj.jpeg')}}" class="img-thumbnail">
                      @else
                      <img src="{{asset('storage/'.$album->image)}}" class="img-thumbnail" style="width:250px; height:300px;">
                      @endif
...
<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal{{$album->id}}">
  ...
  @endif
      ...
      <div class="modal fade" id="exampleModal{{$album->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <form action="{{route('add.album.image')}}" method="POST" enctype="multipart/form-data">
            @csrf
